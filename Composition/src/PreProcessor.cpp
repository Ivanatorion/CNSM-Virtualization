...
std::vector<std::string> PreProcessor::RemoveCommonIncludeDeclarations(AST *program, const std::vector<std::vector<std::string>>& ProgramIncludes){
    static const std::unordered_map<std::string, std::unordered_map<NodeType,std::unordered_map<std::string, int>>> DeleteMap({
        ...
        {NT_EXTERN_DECLARATION, {{"packet_in", 0}, {"packet_out", 0}}},
        {NT_FUNCTION_PROTOTYPE, {{"verify", 1}, {"static_assert", 1}}},
        }},
        {"v1model.p4", {
        ...
        {NT_FUNCTION_PROTOTYPE, {{"invalidate", 1}, {"is_validated", 1}, {"max", 1}, {"min", 1}, {"funnel_shift_right", 1}, {"port_metadata_unpack", 1}, {"sizeInBits", 1}, {"sizeInBytes", 1}}},
        {NT_EXTERN_DECLARATION, {{"Checksum", 0}, {"ParserCounter", 0}, {"ParserPriority", 0}, {"CRCPolynomial", 0}, {"Hash", 0}, {"Random", 0}, {"Counter", 0}, {"Meter", 0}, {"Register", 0}, {"DirectCounter", 0}, {"DirectMeter", 0}, {"DirectRegister", 0}, {"Lpf", 0}, {"Wred", 0}, {"DirectLpf", 0}, {"DirectWred", 0}, {"RegisterParam", 0}, {"RegisterAction", 0}, {"DirectRegisterAction", 0}, {"MathUnit", 0}, 
                                 {"ActionProfile", 0}, {"ActionSelector", 0}, {"SelectorAction", 0}, {"Mirror", 0}, {"Resubmit", 0}, {"Digest", 0}, {"Atcam", 0}, {"Alpm", 0}}},
        {NT_ENUM_DECLARATION, {{"MeterColor_t", 0}, {"MeterType_t", 0}, {"CounterType_t", 0}, {"SelectorMode_t", 0}, {"HashAlgorithm_t", 0}, {"MathOp_t", 0}}},
        }}
    });

    //Remove all parser / control type declarations, and package declarations
    int itr = 0;
    while(itr < (int) program->children.size()){
        if(program->children[itr]->nType == NT_PARSER_TYPE_DECLARATION || program->children[itr]->nType == NT_CONTROL_TYPE_DECLARATION || program->children[itr]->nType == NT_PACKAGE_TYPE_DECLARATION){
            delete program->children[itr];
            program->children.erase(program->children.begin() + itr);
            itr--;
        }
        itr++;
    }
