...
%left '^'
%left '&'
%left TK_OC_PLUSPLUS '+' '-' TK_OC_SAT_ADD TK_OC_SAT_SUB
%left '*' '/' MODULUS
%right PREFIX
%nonassoc ']' '(' '['
%left '.'
...
switchCase switchCases switchLabel nonBraceExpression objDeclaration objDeclarations objInitializer headerStackType

%type<lexval> nonTypeName prefixedNonTypeName name annotationToken member dotPrefix typeName prefixedType nonTableKwName
IDENTIFIER APPLY KEY ACTIONS STATE TYPE_IDENTIFIER THIS ENTRIES TYPE INTEGER STRING_LITERAL FALSE TRUE TK_OC_AND TK_OC_OR TK_OC_LE TK_OC_GE TK_OC_EQ TK_OC_NE TK_OC_PLUSPLUS TK_OC_SAT_ADD TK_OC_SAT_SUB IF ELSE EXIT RETURN SWITCH
...
| optAnnotations EXTERN functionPrototype ';' {delete $2; delete $4; $$ = new AST($3->getValue(), NT_EXTERN_DECLARATION); $$->addChild($1); $$->addChild($3);}
;
...
typedefDeclaration
: optAnnotations TYPEDEF typeRef name ';' {delete $2; delete $5; $$ = new AST($4, NT_TYPEDEF_DECLARATION); $$->addChild($1); $$->addChild($3);}
| optAnnotations TYPEDEF derivedTypeDeclaration name ';' {delete $2; delete $5; $$ = new AST($4, NT_TYPEDEF_DECLARATION); $$->addChild($1); $$->addChild($3);}
;
...
parserDeclaration
: parserTypeDeclaration optConstructorParameters '{' {CSPush();} parserLocalElements parserStates '}' {CSPop(); delete $3; delete $7; $$ = new AST(NULL, NT_PARSER_DECLARATION); $$->addChild($1); $$->addChild($2); $$->addChild($5); $$->addChild($6); $$->setValue($1->getValue());}
;
...
parserState
: optAnnotations STATE name '{' {CSPush();} parserStatements transitionStatement '}' {CSPop(); delete $2; delete $4; delete $8; $$ = new AST($3, NT_PARSER_STATE); $$->addChild($1); $$->addChild($6); $$->addChild($7);}
;
...
valueSetDeclaration
: optAnnotations VALUESET '<' baseType '>' '(' expression ')' name ';' {delete $2; delete $3; delete $5; delete $6; delete $8; delete $10; $$ = new AST($9, NT_VALUE_SET_DECLARATION); $$->addChild($1); $$->addChild($4); $$->addChild($7);}
| optAnnotations VALUESET '<' tupleType '>' '(' expression ')' name ';' {delete $2; delete $3; delete $5; delete $6; delete $8; delete $10; $$ = new AST($9, NT_VALUE_SET_DECLARATION); $$->addChild($1); $$->addChild($4); $$->addChild($7);}
| optAnnotations VALUESET '<' typeName '>' '(' expression ')' name ';' {delete $2; delete $3; delete $5; delete $6; delete $8; delete $10; $$ = new AST($9, NT_VALUE_SET_DECLARATION); $$->addChild($1); $$->addChild(new AST($4, NT_TYPE_NAME)); $$->addChild($7);}
;
...
'{' {CSPush();} controlLocalDeclarations APPLY controlBody '}' {CSPop(); delete $3; delete $6; delete $8; $$ = new AST(NULL, NT_CONTROL_DECLARATION); $$->addChild($1); $$->addChild($2); $$->addChild($5); $$->addChild($7); $$->setValue($1->getValue());}
;
...
| expression '[' expression ']' {delete $2; delete $4; $$ = new AST("[]", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression '[' expression ':' expression ']' {delete $2; delete $6; $$ = new AST("[]", NT_EXPRESSION); $$->addChild($1); AST *rhs = new AST($4, NT_EXPRESSION); rhs->addChild($3); rhs->addChild($5); $$->addChild(rhs);}
| '{' expressionList '}' {delete $1; delete $3; $$ = $2;}
//| '{' kvList '}'
| '(' expression ')' {delete $1; delete $3; $$ = $2;}
| '!' expression %prec PREFIX {$$ = new AST($1, NT_EXPRESSION); $$->addChild($2);}
| '~' expression %prec PREFIX {$$ = new AST($1, NT_EXPRESSION); $$->addChild($2);}
| '-' expression %prec PREFIX {$$ = new AST($1, NT_EXPRESSION); $$->addChild($2);}
| '+' expression %prec PREFIX {$$ = new AST($1, NT_EXPRESSION); $$->addChild($2);}
...
| expression '+' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression '-' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression TK_OC_SAT_ADD expression {delete $2; $$ = new AST("|+|", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression TK_OC_SAT_SUB expression {delete $2; $$ = new AST("|-|", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression TK_OC_LE expression {delete $2; $$ = new AST("<=", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression TK_OC_GE expression {delete $2; $$ = new AST(">=", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression '<' '<' expression {delete $2; delete $3; $$ = new AST("<<", NT_EXPRESSION); $$->addChild($1); $$->addChild($4);}
| expression '>' '>' expression {delete $2; delete $3; $$ = new AST(">>", NT_EXPRESSION); $$->addChild($1); $$->addChild($4);}
| expression '<' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression '>' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
...
| expression '^' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression '|' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| expression TK_OC_PLUSPLUS expression {delete $2; $$ = new AST("++", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
...
| expression '?' expression ':' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); AST *rhs = new AST($4, NT_EXPRESSION); rhs->addChild($3); rhs->addChild($5); $$->addChild(rhs);}
...
| '(' typeRef ')' expression {delete $1; delete $3; $$ = new AST("TYPE_CAST", NT_EXPRESSION); $$->addChild($2); $$->addChild($4);}
;
...
actionList
: /* empty */ {$$ = NULL;}
| actionList optAnnotations actionRef ';' {delete $4; if($1 == NULL) {$$ = new AST(NULL, NT_ACTION_LIST); $$->addChild($2); $$->addChild($3);} else {$$ = $1; $$->addChild($2); $$->addChild($3);}}
;
...
keyElement
: expression ':' name optAnnotations ';' {delete $2; delete $5; $$ = new AST($3, NT_KEY_ELEMENT); $$->addChild($1); $$->addChild($4);}
;
...
| annotations typeRef '(' argumentList ')' name '=' objInitializer ';' {delete $3; delete $5; delete $7; delete $9; $$ = new AST($6, NT_INSTANTIATION); $$->addChild($1); $$->addChild($2); $$->addChild($4); $$->addChild($8);}
| typeRef '(' argumentList ')' name '=' objInitializer ';' {delete $2; delete $4; delete $6; delete $8; $$ = new AST($5, NT_INSTANTIATION); $$->addChild(NULL); $$->addChild($1); $$->addChild($3); $$->addChild($7);}
;

objInitializer
: '{' objDeclarations '}' {delete $1; delete $3; $$ = $2;}
;

objDeclarations
: /* empty */ {$$ = NULL;}
| objDeclarations objDeclaration {if($1 == NULL) {$$ = new AST(NULL, NT_OBJECT_DECLARATION_LIST);} else {$$ = $1;} $$->addChild($2);}
;
...
headerStackType
: typeName '[' expression ']' {delete $2; delete $4; $$ = new AST("[]", NT_HEADER_STACK_TYPE); $$->addChild(new AST($1, NT_TYPE_NAME)); $$->addChild($3);}
| specializedType '[' expression ']' {delete $2; delete $4; $$ = new AST("[]", NT_HEADER_STACK_TYPE); $$->addChild($1); $$->addChild($3);}
;
...
| VARBIT '<' INTEGER '>' {delete $2; delete $4; AST *c = new AST($3, NT_LITERAL); $$ = new AST($1, NT_BASE_TYPE); $$->addChild(c);}
| BIT '<' '(' expression ')' '>' {delete $2; delete $3; delete $5; delete $6; AST *c = $4; c->SimplifyConstExpressions(); $$ = new AST($1, NT_BASE_TYPE); $$->addChild(c);}
| INT '<' '(' expression ')' '>' {delete $2; delete $3; delete $5; delete $6; AST *c = $4; c->SimplifyConstExpressions(); $$ = new AST($1, NT_BASE_TYPE); $$->addChild(c);}
| VARBIT '<' '(' expression ')' '>' {delete $2; delete $3; delete $5; delete $6; AST *c = $4; c->SimplifyConstExpressions(); $$ = new AST($1, NT_BASE_TYPE); $$->addChild(c);}
;
...
/*
kvList
: kvPair {printf("Not implemented!\n"); exit(1);}
| kvList ',' kvPair {printf("Not implemented!\n"); exit(1);}
;

kvPair
: name '=' expression {printf("Not implemented!\n"); exit(1);}
;
*/
...
: IF '(' expression ')' statement {delete $1; delete $2; delete $4; $$ = new AST("IF", NT_CONDITIONAL_STATEMENT); $$->addChild($3); $$->addChild($5); $$->addChild(NULL);} %prec "then"
| IF '(' expression ')' statement ELSE statement {delete $1; delete $2; delete $4; delete $6; $$ = new AST("IF", NT_CONDITIONAL_STATEMENT); $$->addChild($3); $$->addChild($5); if($7->getNodeType() != NT_STAT_OR_DEC_LIST) {AST *s = $7; $7 = new AST(NULL, NT_STAT_OR_DEC_LIST); $7->addChild(s);} $$->addChild($7);}
;
...
| switchLabel ':' {delete $2; $$ = new AST(NULL, NT_SWITCH_CASE); $$->addChild($1);}
;
...
| nonTypeName {$$ = new AST($1, NT_EXPRESSION); CSSubstitute(&($$));}
//| dotPrefix nonTypeName
| nonBraceExpression '[' expression ']' {delete $2; delete $4; $$ = new AST("[]", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '[' expression ':' expression ']' {delete $2; delete $6; $$ = new AST("[]", NT_EXPRESSION); $$->addChild($1); AST *rhs = new AST($4, NT_EXPRESSION); rhs->addChild($3); rhs->addChild($5); $$->addChild(rhs);}
| '(' expression ')' {delete $1; delete $3; $$ = $2;}
| '!' expression %prec PREFIX {$$ = new AST($1, NT_EXPRESSION); $$->addChild($2);}
| '~' expression %prec PREFIX {$$ = new AST($1, NT_EXPRESSION); $$->addChild($2);}
| '-' expression %prec PREFIX {$$ = new AST($1, NT_EXPRESSION); $$->addChild($2);}
...
| typeName '.' member {$$ = new AST($2, NT_EXPRESSION); $$->addChild(new AST($1, NT_NONE)); $$->addChild(new AST($3, NT_NONE));}
//| ERROR '.' member
| nonBraceExpression '.' member {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild(new AST($3, NT_NONE));}
| nonBraceExpression '*' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '/' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression MODULUS expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '+' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '-' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_SAT_ADD expression {delete $2; $$ = new AST("|+|", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_SAT_SUB expression {delete $2; $$ = new AST("|-|", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_LE expression {delete $2; $$ = new AST("<=", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_GE expression {delete $2; $$ = new AST(">=", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '<' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '>' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_NE expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_EQ expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '<' '<' expression {delete $2; delete $3; $$ = new AST("<<", NT_EXPRESSION); $$->addChild($1); $$->addChild($4);}
| nonBraceExpression '>' '>' expression {delete $2; delete $3; $$ = new AST(">>", NT_EXPRESSION); $$->addChild($1); $$->addChild($4);}
| nonBraceExpression '&' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '^' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '|' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_PLUSPLUS expression {delete $2; $$ = new AST("++", NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_AND expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression TK_OC_OR expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); $$->addChild($3);}
| nonBraceExpression '?' expression ':' expression {$$ = new AST($2, NT_EXPRESSION); $$->addChild($1); AST *rhs = new AST($4, NT_EXPRESSION); rhs->addChild($3); rhs->addChild($5); $$->addChild(rhs);}
| nonBraceExpression '<' realTypeArgumentList '>' '(' argumentList ')' {delete $2; delete $4; delete $5; delete $7; $$ = $1; $$->addChild($3); $$->addChild($6);}
| nonBraceExpression '(' argumentList ')' {delete $2; delete $4; $$ = $1; $$->setNodeType(NT_EXPRESSION); $$->addChild($3);}
| namedType '(' argumentList ')' {delete $2; delete $4; $$ = $1; $$->setNodeType(NT_EXPRESSION); $$->addChild($3);}
| '(' typeRef ')' expression {delete $1; delete $3; $$ = new AST("TYPE_CAST", NT_EXPRESSION); $$->addChild($2); $$->addChild($4);}
;
...
: lvalue '(' argumentList ')' ';' {delete $2; delete $4; delete $5; $$ = new AST(NULL, NT_METHOD_CALL_STATEMENT); $$->addChild($1); $$->addChild(NULL); $$->addChild($3);}
| lvalue '<' typeArgumentList '>' '(' argumentList ')' ';' {delete $2; delete $4; delete $5; delete $7; delete $8; $$ = new AST(NULL, NT_METHOD_CALL_STATEMENT); $$->addChild($1); $$->addChild($3); $$->addChild($6);}
...
| lvalue '[' expression ':' expression ']' {delete $2; delete $6; $$ = new AST("[]", NT_NONE); $$->addChild($1); AST *rhs = new AST($4, NT_EXPRESSION); rhs->addChild($3); rhs->addChild($5); $$->addChild(rhs);}
;
